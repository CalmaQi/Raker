...
# class BertForKBCSequenceClassification(MPNetPreTrainedModel):
    def __init__(self, config):
        ...
        # self.type_relation = torch.nn.Embedding(self.ntoken *4, config.hidden_size)
        # LSTM
    ...
    def forward(
        ...
        # torch.relu(pooled_output) 
        #relu后接fc
  
        # ipdb.set_trace()
        #对每个triple都有邻居triple那么输入的数据 batch_size,nei_num,input_ids
        
    
        # 真实三元组embedding：batch_size,contex_num,dim 
        # batch_size=len(context_text['input_ids'])
        # # context_num,max_len=context_text['input_ids'][0].shape
        # all_batch_context=[]
        ...
        # #         return_dict=True)
        # # ipdb.set_trace()
        # cross_attention=self.crossencoder(output1[0],output2[0])
        # cross_attention=self.classifier(self.dropout(cross_attention[0][:,0]))
        # ipdb.set_trace()
        
        # cross_attention=self.bert2(inputs_embeds=output1[1].unsqueeze(1),encoder_hidden_states=output2[1].unsqueeze(1), return_dict=True)
        # cross_attention=self.classifier2(self.dropout(cross_attention[1]))
        # for batch_iter in range(batch_size) :
        #     output_tar = self.bert(
        #         input_ids=context_text['input_ids'][batch_iter],
        #         attention_mask=context_text['attention_mask'][batch_iter],
        #         return_dict=True)
        #     input_ids=context_text['input_ids'][batch_iter][0]
        #     attention_mask=context_text['attention_mask'][batch_iter][0],
            
        #     ipdb.set_trace()
        #     all_batch_context.append(output_tar[1])
        # #all_batch_context=torch.cat(all_batch_context)    
        
        # ipdb.set_trace()
      
       # output_tar_cls     batch_size,nei_num,dim
        
        # context_output=self.contextlayer(all_batch_context)
       
        
        # context_output=torch.cat([context_output.squeeze(1),pooled_output],dim=1)
        logits = self.classifier(pooled_output)
        
        # logits = self.classifier(pooled_output).unsqueeze(1)
        
        # # 0.1 wn18rr:0.01
        # logits = cross_attention
        
        # predict = torch.cat([cross_attention.unsqueeze(1)*0.3, logits], dim=1)
        # # # ipdb.set_trace()
        # weight = torch.softmax(0.5 * predict, dim=1)
        # # logits = (predict * weight.detach()).sum(1).sigmoid()
        # logits = (predict * weight.detach()).sum(1)
        # ipdb.set_trace()
        # logits=torch.cat([(1-cossim).unsqueeze(1),cossim.unsqueeze(1)],dim=1)
